...
import { HelpCircle, Save, AlertCircle, FileText, Download, Lock } from 'lucide-react';
...
function App() {
  const [inputs, setInputs] = useState({
    ...
    // Applied loads - Fixed values for restricted version
    vx: 0, // kN
    vy: 0, // kN
    tb: 0, // kNm (torsion)
    mb: 50, // kNm (major axis moment)
    mm: 0, // kNm (minor axis moment)
    nt: 0, // kN (axial force)
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Prevent changes to restricted inputs
    if (['vx', 'vy', 'tb', 'mm', 'nt', 'boltSize'].includes(name)) {
      return;
    }
  ...
  const renderLockedInput = (label: string, value: number | string, unit: string = '') => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
              Bolt Group Analysis Calculator (Free Version)
            </h1>
          </div>
        </div>
      </header>
        ...
        <div className="bg-white rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 p-6 mb-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-4">
              ...
              {renderLockedInput("Horizontal Shear (Vx)", inputs.vx, " kN")}
              {renderLockedInput("Vertical Shear (Vy)", inputs.vy, " kN")}
              {renderLockedInput("Torsion (Tb)", inputs.tb, " kNm")}
              ...
              {renderLockedInput("Minor Axis Moment (Mm)", inputs.mm, " kNm")}
              {renderLockedInput("Axial Force (Nt)", inputs.nt, " kN")}
            </div>
            ...
            <div className="space-y-4">
              ...
              {renderLockedInput("Bolt Size", inputs.boltSize)}
